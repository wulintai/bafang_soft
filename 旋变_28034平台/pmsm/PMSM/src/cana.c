...
{
	if(EV_MCU_Para.field.TrqCmd_SourceType == TRQCMD_SOURCE_ECANA)
	{
		...
		{
			if((emsw2.field.can_fault == 1)&&(sccw2.field.fault_recover == 1))
			{
	 ...
	 //if(0 == can_debugmode)
	 {
		if ((ECanaRegs.CANRMP.all & 0x0001)==0x001)
		{
...
{
	ECanaMBoxData[18].MDL.word.LOW_WORD     	=   crtLoopPara.Id_ref_HMI;
	ECanaMBoxData[18].MDL.word.HI_WORD      	=   crtLoopPara.Iq_ref_HMI;
	ECanaMBoxData[18].MDH.word.LOW_WORD         =   sysFbkPara.Id_fbk_HMI;
	ECanaMBoxData[18].MDH.word.HI_WORD      	=   sysFbkPara.Iq_fbk_HMI;
}

void packcanaMbx19(void)//4186
{
	ECanaMBoxData[19].MDL.word.LOW_WORD     	=   voltLoopPara.Vd_ref_HMI>>1;
	ECanaMBoxData[19].MDL.word.HI_WORD      	=   voltLoopPara.Vq_ref_HMI>>1;
	ECanaMBoxData[19].MDH.word.LOW_WORD      =   voltLoopPara.Vdq_HMI>>1;
	ECanaMBoxData[19].MDH.word.HI_WORD      	=   sysCfgPara.Vdc_HMI;
}

void packcanaMbx20(void)//4187
{
	if(ELECTRIC_MODE == trqLoop_EleGenMode)
	{
		ECanaMBoxData[20].MDL.word.LOW_WORD     	=   trqLoopPara.Max_TorqueEle_HMI;
	}
	else
	{
		ECanaMBoxData[20].MDL.word.LOW_WORD     	=   trqLoopPara.Max_TorqueGen_HMI;
	}
    ECanaMBoxData[20].MDL.word.HI_WORD      	=   EV_MCU_Para.field.Motor_Encoder_offset;
	ECanaMBoxData[20].MDH.word.LOW_WORD      =   _IQtoIQ15(sysCfgPara.SpdCmd);
	ECanaMBoxData[20].MDH.word.HI_WORD      	=   sysFbkPara.Speed_fbk_HMI;
}
...
void packcanaMbx21(void)//4188
{
	...
	ECanaMBoxData[21].MDL.word.HI_WORD      =   sysFbkPara.Temp_igbt;
	ECanaMBoxData[21].MDH.word.LOW_WORD      =   sysFbkPara.Idc_fbk_HMI;
	ECanaMBoxData[21].MDH.word.HI_WORD      	=   sysFbkPara.Is_fbk_HMI_fiter;
}
...
{
	ECanaMBoxData[23].MDL.word.LOW_WORD     	=   (_IQtoIQ15(mtrfwdvars.IdPid_Out_fwd))>>1;
	ECanaMBoxData[23].MDL.word.HI_WORD      	=   (_IQtoIQ15(mtrfwdvars.IqPid_Out_fwd))>>1;
	ECanaMBoxData[23].MDH.word.LOW_WORD         =   sysFbkPara.Temp_mtr;
	ECanaMBoxData[23].MDH.word.HI_WORD      	=   vehicle_io_state.wValue;//_IQtoIQ15(trqidqmap_absSpdFbk);
}
void packcanaMbx24(void)//418B
{
	ECanaMBoxData[24].MDL.word.LOW_WORD     	=   emsw.wValue;
	ECanaMBoxData[24].MDL.word.HI_WORD      	=   emsw1.wValue;
	ECanaMBoxData[24].MDH.word.LOW_WORD         =   emsw2.wValue;
	ECanaMBoxData[24].MDH.word.HI_WORD      	=   alarm.wValue;
}
void packcanaMbx25(void)//418C
{
	ECanaMBoxData[25].MDL.word.LOW_WORD     	=   trqLoopPara.Torque_ref_HMI;
	
	Motor_VarPara.field.DI = vehicle_io_state.wValue;
	Motor_VarPara.field.IU_AD = sysFbkPara.Iu_fbk_ad;
	Motor_VarPara.field.IV_AD = sysFbkPara.Iv_fbk_ad;
	Motor_VarPara.field.IW_AD = sysFbkPara.Iw_fbk_ad;
	Motor_VarPara.field.VPN_AD = sysFbkPara.Vdc_fbk_ad;
	Motor_VarPara.field.OV_REF = 0;
	Motor_VarPara.field.T_IGBT_AD = sysFbkPara.Temp_mdl_ad;
	Motor_VarPara.field.AI3_AD = 0;
	Motor_VarPara.field.TEMP_M2_AD = 0;
	Motor_VarPara.field.TEMP_M1_AD = sysFbkPara.Temp_mtr_ad;
	Motor_VarPara.field.V12_ADC = 0;
	Motor_VarPara.field.AI1_AD = sysFbkPara.AI1_fbk_ad;
	Motor_VarPara.field.V5_ADC = 0;
	Motor_VarPara.field.AI2_AD = sysFbkPara.AI2_fbk_ad;
	Motor_VarPara.field.V5_fbk_HMI = 0;
	Motor_VarPara.field.V12_fbk_HMI = sysFbkPara.V12_fbk_HMI;
	...
	ECanaMBoxData[25].MDH.word.HI_WORD         	=   sysCfgPara.Theta_ele_HMI;
}

void packcanaMbx26(void)//418D
{
	ECanaMBoxData[26].MDL.word.LOW_WORD         =   sysFbkPara.AI1_fbk_ad;
	ECanaMBoxData[26].MDL.word.HI_WORD          =   sysFbkPara.AI2_fbk_ad;
	ECanaMBoxData[26].MDH.word.LOW_WORD         =   _IQtoIQ15(spdLoopPara.Iqcmd_real);
	ECanaMBoxData[26].MDH.word.HI_WORD          =   _IQtoIQ15(spdLoopPara.Ui);
}
void packcanaMbx27(void)//418E
{
	ECanaMBoxData[27].MDL.word.LOW_WORD         =   _IQtoIQ15(crtLoopPara.Id_PI_Vd);
	ECanaMBoxData[27].MDL.word.HI_WORD          =   _IQtoIQ15(crtLoopPara.IdPid_Ui);
	ECanaMBoxData[27].MDH.word.LOW_WORD         =   _IQtoIQ15(crtLoopPara.Iq_PI_Vq);
	ECanaMBoxData[27].MDH.word.HI_WORD          =   _IQtoIQ15(crtLoopPara.IqPid_Ui);
}

void packcanaMbx28(void)//418F
{
	ECanaMBoxData[28].MDL.word.LOW_WORD = _IQtoIQ15(weakcrtLoopvars.weakIdPid_Outfilter);
	ECanaMBoxData[28].MDL.word.HI_WORD  = _IQtoIQ15(weakcrtLoopvars.weakIdPid_Ui);
	ECanaMBoxData[28].MDH.word.LOW_WORD = diagEncoffsetVar.Motor_Encoder_offset>>12;
	ECanaMBoxData[28].MDH.word.HI_WORD = trqLoopPara.Torque_fbk_HMI;
#if 0
...
void packcanaMbx11(void)//4192
{
	...
	ECanaMBoxData[11].MDH.word.LOW_WORD 		=	sysFbkPara.Temp_ta;
	ECanaMBoxData[11].MDH.word.HI_WORD			=	sysFbkPara.AI1_fbk_ad;//ipark_angle_str.Ipark_angle_out_filter>>12;
}
...
void packcanaMbx8(void)//0x18ef0103 控制器-->仪表
{
	...
	{
		if(0xa0 == ECanaMBoxData[8].MDL.byte.BYTE3)//发送电机转速、车速
		{
			ECanaMBoxData[8].MDL.byte.BYTE3 = 0xa0;
			ECanaMBoxData[8].MDL.byte.BYTE2 = 0x07;
			//电机转速
			rpm = sysFbkPara.Speed+10000;
			ECanaMBoxData[8].MDL.byte.BYTE1 = ((rpm>>8)&0xFF);
			ECanaMBoxData[8].MDL.byte.BYTE0 = (rpm&0xFF);
			...
			speed = _IQ15div(_IQ15mpy(_IQ15(1.2064),_IQ15(sysFbkPara.Speed_abs)),speed_rate);//IQ15
			speed = speed>>15;//IQ0
		...
		else//发送开关量
		{
			...
			canafbk_state.data.gear_state = Vehicle_cmd.cmdmode.data.gear_state;
			canafbk_state.data.brk_state = Vehicle_cmd.cmdmode.data.brake_state;
			canafbk_state.data.drive_state = ((vehicle_io_state.data.DI1_WAKEUP_DSP)|(vehicle_io_state.data.DI3_DSP<<1));
	...
	else if(0x07 == ECanaMBoxData[8].MDL.byte.BYTE2)//发送电控电流
	{
		...
		//电控电流
		current = _IQ15mpy(_IQ15div(sysFbkPara.Is_fbk_HMI_fiter,_IQ15(1.414214)),SysBase.iphase);
	...
	else if(0x08 == ECanaMBoxData[8].MDL.byte.BYTE2)//动力电池电量
	{
		...
		ECanaMBoxData[8].MDH.word.LOW_WORD = 0x0000;
		ECanaMBoxData[8].MDH.byte.BYTE5 = canarev_cmd.soc_ref;
	...
	else//发送故障
	{
		...
		if(1 == alarm.field.udc_lv)
		{
			MCU_err_status1.data.udc_lackvolt = 1;
		}
		if(1 == alarm.field.udc_ov)
		{
		...
		ECanaMBoxData[8].MDL.word.HI_WORD = MCU_err_status1.wValue;

		MCU_err_status2.wValue = 0x0000;
		if(1 == emsw.field.eeprom_fault)
		{
			MCU_err_status2.data.eeprom_fault = 1;
		}
		if(1 == emsw2.field.can_fault)
		{
			MCU_err_status2.data.can_fault = 1;
		}
		if(1 == emsw2.field.DIAG_ENCODER_error)
		{
			MCU_err_status2.data.diagstudy_error = 1;
		}
		if((1 == emsw1.field.mtrsensor_short_error)
		 ||(1 == emsw1.field.mtrsensor_open_error))
		{
			MCU_err_status2.data.mtr_sensor_fault = 1;
		}
		if((1 == emsw1.field.V12_lackVolt)
		 ||(1 == emsw1.field.V12_overVolt))
		{
			MCU_err_status2.data.v12_fault = 1;
		}
		ECanaMBoxData[8].MDH.word.LOW_WORD = MCU_err_status2.wValue;

		MCU_err_status3.wValue = 0x0000;
		ECanaMBoxData[8].MDH.word.HI_WORD = MCU_err_status3.wValue;
	}
}
...
void packcanaMbx9(void)//0x18ef0301 控制器-->仪表
{
	...
	ECanaMBoxData[9].MDH.byte.BYTE7 = ((canafbk_state.wValue>>8)&0x03);
...
void canaMbxSlotCtrl(void)
{
	...
	for(cana_index=0;cana_index<21;cana_index++)
	{
		if(1 == canambxCfg[cana_index].txEnable)
		{
			canambxCfg[cana_index].cnt++;
			if(canambxCfg[cana_index].cnt>=canambxCfg[cana_index].prd)
			{
				canambxCfg[cana_index].flag = 1;
				canambxCfg[cana_index].cnt = 0;
			}
		}
		else
		{
			canambxCfg[cana_index].flag = 0;
			canambxCfg[cana_index].cnt = 0;
		}
	}
}
...
{
	canaMbxSlotCtrl();
	if((1 == canambxCfg[0].txEnable) && (1 == canambxCfg[0].flag))
	{
		packcanaMbx8();
		canTransmitDriver(CAN_A_INDEX, 8, &ECanaMBoxData[8]);
		canambxCfg[0].flag=0;
	}
